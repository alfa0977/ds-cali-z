    ...
    <div className="flex h-full flex-col bg-background">
      ...
      <div className="relative mx-4 flex-1 overflow-hidden rounded-3xl bg-black">
        {image ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-3 text-white/70">
            ...
            <p className="text-sm">Point your camera at your meal</p>
          </div>
        )}
        ...
        {analyze.isPending && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/50 backdrop-blur-sm">
            ...
            <p className="text-sm font-medium text-white">Analyzing your meal…</p>
          </div>
        )}
        ...
        {!image && !analyze.isPending && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2">
            <div className="flex items-center gap-2 rounded-full bg-white/95 px-4 py-2 shadow-lg">
              ...
              <span className="text-sm font-semibold text-foreground">Scan food</span>
            </div>
          </div>
        )}
      ...
      {!image && (
        <div className="px-4 py-3">
          <p className="mb-2 text-xs font-medium text-muted-foreground">Try a sample meal:</p>
  ...
  return (
    <div className="border-t border-border bg-card">
        ...
        <div className="flex flex-wrap gap-1.5">
          {analysis.ingredients.map((ing, i) => (
            <span key={i} className="rounded-full bg-secondary px-2.5 py-1 text-[11px] font-medium">
              {ing.name} {Math.round(ing.estimatedWeightGrams)}g
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 py-4">
        <p className="text-xs text-muted-foreground">{analysis.detectedCategory ?? "Meal"}</p>
        <h2 className="text-xl font-bold">{analysis.mealTitle ?? "Scanned meal"}</h2>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <NutBox label="Calories" value={scaledMacros.calories} unit="" icon="🔥" color="text-streak" />
          <NutBox label="Carbs" value={scaledMacros.carbs} unit="g" icon="🌾" color="text-carbs" />
          <NutBox label="Protein" value={scaledMacros.protein} unit="g" icon="🍗" color="text-protein" />
          <NutBox label="Fats" value={scaledMacros.fat} unit="g" icon="💧" color="text-fats" />
        </div>
        ...
        <div className="mt-3 flex items-center gap-2 rounded-2xl bg-secondary p-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <span className="text-sm font-bold tabular-nums">{analysis.healthScore}/100</span>
          </div>
        </div>
        ...
        {editing && (
          <div className="mt-4 space-y-2">
            <p className="text-xs font-semibold text-muted-foreground">Edit ingredients</p>
