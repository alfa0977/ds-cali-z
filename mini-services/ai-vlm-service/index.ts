// DS-Cali AI VLM Service
// A small Bun server that wraps the z-ai-web-dev-sdk VLM for meal analysis.
// The APK can call this service via HTTP when the "Remote Z-AI service" engine is selected.
//
// Usage:
...
//   bun run dev
//
// The server listens on port 3031.
// In the APK, set the "Remote service URL" to:
//   /api/analyze?XTransformPort=3031
// (The gateway forwards this to the service.)
//
// For production deployment, deploy this service to a public server and set the full URL.
...
const server = Bun.serve({
  port: PORT,
  async fetch(req) {
    // CORS headers for cross-origin requests from the APK
    const headers = {
